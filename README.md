...
The dataset is composed of 150 pages, with 50 pages of each type of data. Its developed presentation is accessible in the csv file, where each page is described. The schema below explains its process of creation. The pages have been prepared and segmented in [eScriptorium](http://traces6.paris.inria.fr/) using the [SegmOnto](https://github.com/SegmOnto) ontology which allows to name the differents zones and lines. Lastly, the work done has been exported in PageXML and ALTO4 format, accessible in this repository, along with the images used. 
